...
const prepareAdditionalData = (addtionalData) => {
    const strs = addtionalData.map(([name,type]) => {
        const generic = type.slice(0,1).toUpperCase()
        return [`${name}: ${generic}`,`${generic} extends ${type}`]
    })
    const [names,types] = strs.reduce((acc,cur) => [[...acc[0],cur[0]],[...acc[1],cur[1]]],[[],[]])
...
function write(obj = { Ziro }) {
    return Object.keys(obj).map(key => {
        if(Array.isArray(obj[key])) {
            ...
            const array = Array.from(Array(upperBound-lowerBound+1).keys()).map(n => {
            ...
            return `\nexport type ${key}Codes=${array}\nexport type ${key}Data<C extends ${key}Codes${addData[1]}> = ${key.includes("Waiting") ? "ZiroWaitingFullData":"ZiroPromptFullData"}<C${addData[0]}>`
        }
        else return `/**\n * ${key.toUpperCase()}\n */\nexport type ${key}Codes=${Object.keys(obj[key]).map(k => k+"Codes").join("|")}\n`+write(obj[key])
    }).join("\n")
}

const str = header+write()

fs.writeFileSync(__dirname+"/index.ts",str)